...
func RegisterSubclass[T any]() *RegisteredSubclass[T] {
	...
	return RegisterSubclassWithConstructor[T](func() T {
...
func extractParentType(rtype reflect.Type) SubclassTypeInfo {
	...
	typeInfo, ok := subclassTypeInfos[field.Type]
	if !ok {
...
// SubclassTypeInfo contains autogenerated data made by the generator that binds
// relevant subclassing information.
type SubclassTypeInfo struct {
	GType        Type
	GoType       reflect.Type
	ClassSize    uint16 // ushort, size of the ObjectClass type
	InstanceSize uint16 // ushort, size of the Object type
...
var subclassTypeInfos map[reflect.Type]SubclassTypeInfo

// RegisterSubclassTypeInfo registers the given subclass type info.
func RegisterSubclassTypeInfo(typeInfo SubclassTypeInfo) {
	subclassTypeInfos[typeInfo.GoType] = typeInfo
}
...
// GoInstanceFromPrivateInstance returns the Go value from a given GObject
// pointer whose type is that of a Go subclass type. This function is only ever
// used internally.
func GoInstanceFromPrivateInstance(instance unsafe.Pointer) any {
