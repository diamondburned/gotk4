...
func NewPixbufFromXPMData(data []string) *Pixbuf {
	...
	{
		_arg1 = (**C.char)(C.malloc(C.size_t(len(data)+1) * C.size_t(unsafe.Sizeof(uint(0)))))
		defer C.free(unsafe.Pointer(_arg1))
		{
...
func (pixbuf *Pixbuf) SaveToBufferv(typ string, optionKeys, optionValues []string) ([]byte, error) {
	...
	{
		_arg4 = (**C.char)(C.malloc(C.size_t(len(optionKeys)+1) * C.size_t(unsafe.Sizeof(uint(0)))))
	...
	{
		_arg5 = (**C.char)(C.malloc(C.size_t(len(optionValues)+1) * C.size_t(unsafe.Sizeof(uint(0)))))
...
func (pixbuf *Pixbuf) SaveToStreamv(ctx context.Context, stream gio.OutputStreamer, typ string, optionKeys, optionValues []string) error {
	...
	{
		_arg3 = (**C.char)(C.malloc(C.size_t(len(optionKeys)+1) * C.size_t(unsafe.Sizeof(uint(0)))))
		defer C.free(unsafe.Pointer(_arg3))
		{
	...
	{
		_arg4 = (**C.char)(C.malloc(C.size_t(len(optionValues)+1) * C.size_t(unsafe.Sizeof(uint(0)))))
		defer C.free(unsafe.Pointer(_arg4))
		{
...
func (pixbuf *Pixbuf) SaveToStreamvAsync(ctx context.Context, stream gio.OutputStreamer, typ string, optionKeys, optionValues []string, callback gio.AsyncReadyCallback) {
	...
	defer C.free(unsafe.Pointer(_arg2))
	{
		_arg3 = (**C.gchar)(C.malloc(C.size_t(len(optionKeys)+1) * C.size_t(unsafe.Sizeof(uint(0)))))
		defer C.free(unsafe.Pointer(_arg3))
		{
			out := unsafe.Slice(_arg3, len(optionKeys)+1)
			var zero *C.gchar
	...
	{
		_arg4 = (**C.gchar)(C.malloc(C.size_t(len(optionValues)+1) * C.size_t(unsafe.Sizeof(uint(0)))))
		defer C.free(unsafe.Pointer(_arg4))
		{
			out := unsafe.Slice(_arg4, len(optionValues)+1)
			var zero *C.gchar
