...
{
arg2 = (*C.C.GDBusErrorEntry)(C.malloc(C.sizeof_GDBusErrorEntry * a))
for i := 0; i < uintptr(a); i++ {
  src := entries[i]
  var dst C.C.GDBusErrorEntry
  dst = (C.C.GDBusErrorEntry)(src.Native())
  *(*C.C.GDBusErrorEntry)(unsafe.Pointer(uintptr(unsafe.Pointer(arg2))+i)) = dst
}
}
