...
// NewSettingsFull creates a new #GSettings object with a given schema, backend
// and path.
//
// It should be extremely rare that you ever want to use this function.
// It is made available for advanced use-cases (such as plugin systems that want
// to provide access to schemas loaded from custom locations, etc).
//
// At the most basic level, a #GSettings object is a pure composition of 4
// things: a Schema, a Backend, a path within that backend, and a Context to
// which signals are dispatched.
//
// This constructor therefore gives you full control over constructing
// #GSettings instances. The first 3 parameters are given directly as schema,
// backend and path, and the main context is taken from the thread-default (as
// per g_settings_new()).
//
// If backend is NULL then the default backend is used.
//
// If path is NULL then the path from the schema is used. It is an error if path
// is NULL and the schema has no path of its own or if path is non-NULL and not
// equal to the path that the schema does have.
//
// The function takes the following parameters:
//
//   - schema: Schema.
//   - backend (optional): Backend.
//   - path (optional) to use.
//
// The function returns the following values:
//
//   - settings: new #GSettings object.
//
func NewSettingsFull(schema *SettingsSchema, backend SettingsBackender, path string) *Settings {
	var _arg1 *C.GSettingsSchema  // out
	var _arg2 *C.GSettingsBackend // out
	var _arg3 *C.gchar            // out
	var _cret *C.GSettings        // in

	_arg1 = (*C.GSettingsSchema)(gextras.StructNative(unsafe.Pointer(schema)))
	if backend != nil {
		_arg2 = (*C.GSettingsBackend)(unsafe.Pointer(coreglib.InternObject(backend).Native()))
	}
	if path != "" {
...
// NewSettingsWithBackend creates a new #GSettings object with the schema
// specified by schema_id and a given Backend.
//
// Creating a #GSettings object with a different backend allows accessing
// settings from a database other than the usual one. For example, it may make
// sense to pass a backend corresponding to the "defaults" settings database on
// the system to get a settings object that modifies the system default settings
// instead of the settings for this user.
//
// The function takes the following parameters:
//
//   - schemaId: id of the schema.
...
//   - settings: new #GSettings object.
//
func NewSettingsWithBackend(schemaId string, backend SettingsBackender) *Settings {
	var _arg1 *C.gchar            // out
	var _arg2 *C.GSettingsBackend // out
	var _cret *C.GSettings        // in
...
// NewSettingsWithBackendAndPath creates a new #GSettings object with the schema
// specified by schema_id and a given Backend and path.
//
...
//   - settings: new #GSettings object.
//
func NewSettingsWithBackendAndPath(schemaId string, backend SettingsBackender, path string) *Settings {
	var _arg1 *C.gchar            // out
	var _arg2 *C.GSettingsBackend // out
	var _arg3 *C.gchar            // out
	var _cret *C.GSettings        // in
