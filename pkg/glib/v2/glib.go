...
// ChildWatchFunc: prototype of a WatchSource callback, called when a
// child process has exited. To interpret status, see the documentation for
// g_spawn_check_exit_status().
type ChildWatchFunc func(pid coreglib.PID, status int)
...
//
// If you obtain pid from g_spawn_async() or g_spawn_async_with_pipes() you will
// need to pass SPAWN_DO_NOT_REAP_CHILD as flag to the spawn function for the
...
// Typically, you should invoke g_spawn_close_pid() in the callback function for
// the source.
//
// GLib supports only a single callback per process id. On POSIX platforms,
// the same restrictions mentioned for g_child_watch_source_new() apply to this
// function.
//
...
//   - pid process to watch. On POSIX the positive pid of a child process.
//     On Windows a handle for a process (which doesn't have to be a child).
//   - function to call.
//
// The function returns the following values:
//
...
//
func ChildWatchAdd(priority int, pid coreglib.PID, function ChildWatchFunc) uint {
	var _arg1 C.gint            // out
	var _arg2 C.GPid            // out
	var _arg3 C.GChildWatchFunc // out
	...
	_arg2 = C.GPid(pid)
	_arg3 = (*[0]byte)(C._gotk4_glib2_ChildWatchFunc)
	_arg4 = C.gpointer(gbox.Assign(function))
	_arg5 = (C.GDestroyNotify)((*[0]byte)(C.callbackDelete))

	_cret = C.g_child_watch_add_full(_arg1, _arg2, _arg3, _arg4, _arg5)
...
// The function returns the following values:
//
//   - source: newly-created child watch source.
//
func NewChildWatchSource(pid coreglib.PID) *Source {
...
// its windows on the right screen, you may want to use AppLaunchContext,
...
// The function returns the following values:
//
//   - childPid (optional): return location for child process reference,
//     or NULL.
//
func SpawnAsync(workingDirectory string, argv, envp []string, flags SpawnFlags, childSetup SpawnChildSetupFunc) (coreglib.PID, error) {
	var _arg1 *C.gchar               // out
	var _arg2 **C.gchar              // out
	var _arg3 **C.gchar              // out
	...
	var _childPid coreglib.PID // out
	var _goerr error           // out

	_childPid = coreglib.PID(_arg7)
	if _cerr != nil {
		_goerr = gerror.Take(unsafe.Pointer(_cerr))
	}
...
// The function returns the following values:
//
//   - childPid (optional): return location for child process ID, or NULL.
//
func SpawnAsyncWithFds(workingDirectory string, argv, envp []string, flags SpawnFlags, childSetup SpawnChildSetupFunc, stdinFd, stdoutFd, stderrFd int) (coreglib.PID, error) {
	var _arg1 *C.gchar               // out
	var _arg2 **C.gchar              // out
	var _arg3 **C.gchar              // out
	var _arg4 C.GSpawnFlags          // out
...
//
//   - childPid (optional): return location for child process ID, or NULL.
//   - standardInput (optional): return location for file descriptor to write to
//     child's stdin, or NULL.
//   - standardOutput (optional): return location for file descriptor to read
//     child's stdout, or NULL.
//   - standardError (optional): return location for file descriptor to read
//     child's stderr, or NULL.
//
func SpawnAsyncWithPipes(workingDirectory string, argv, envp []string, flags SpawnFlags, childSetup SpawnChildSetupFunc) (childPid coreglib.PID, standardInput, standardOutput, standardError int, goerr error) {
	var _arg1 *C.gchar               // out
	var _arg2 **C.gchar              // out
	var _arg3 **C.gchar              // out
	var _arg4 C.GSpawnFlags          // out
...
// SpawnAsyncWithPipesAndFds executes a child program asynchronously
// (your program will not block waiting for the child to exit). The child
// program is specified by the only argument that must be provided, argv.
// argv should be a NULL-terminated array of strings, to be passed as the
// argument vector for the child. The first string in argv is of course
// the name of the program to execute. By default, the name of the program
// must be a full path. If flags contains the G_SPAWN_SEARCH_PATH flag,
// the PATH environment variable is used to search for the executable. If flags
// contains the G_SPAWN_SEARCH_PATH_FROM_ENVP flag, the PATH variable from envp
...
//
// If the program name is not a full path and G_SPAWN_SEARCH_PATH flag is
// not used, then the program will be run from the current directory (or
...
// KEY=VALUE. This will become the child's environment. If envp is NULL,
...
// automatically be reaped; you must use a child watch (g_child_watch_add())
// to be notified about the death of the child process, otherwise it will stay
// around as a zombie process until this process exits. Eventually you must
// call g_spawn_close_pid() on the child_pid, in order to free resources which
// may be associated with the child process. (On Unix, using a child watch is
...
// all descriptors except stdin/stdout/stderr will be closed before calling
// exec() in the child. G_SPAWN_SEARCH_PATH means that argv[0] need not be
// an absolute path, it will be looked for in the PATH environment variable.
...
// a single FD for both stdout_fd and stderr_fd, and include it in source_fds
// too).
//
...
// target_fds. The source and target FD may be equal to simply propagate an
// FD to the spawned process. FD remappings are processed after standard FDs,
// so any target FDs which equal stdin_fd, stdout_fd or stderr_fd will overwrite
...
// the function is called in the child after GLib has performed all the setup
// it plans to perform (including creating pipes, closing file descriptors,
// etc.) but before calling exec(). That is, child_setup is called just before
...
// There is no sensible thing child_setup could be used for on Windows so it is
...
// occur in the child (for example if the executable in argv[0] is not found).
// Typically the message field of returned errors should be displayed to users.
// Possible errors are those from the SPAWN_ERROR domain.
//
...
//   - stdoutFd: file descriptor to use for child's stdout, or -1.
//   - stderrFd: file descriptor to use for child's stderr, or -1.
//   - sourceFds (optional): array of FDs from the parent process to make
//     available in the child process.
//   - targetFds (optional): array of FDs to remap source_fds to in the child
//     process.
//
// The function returns the following values:
//
//   - childPidOut (optional): return location for child process ID, or NULL.
//   - stdinPipeOut (optional): return location for file descriptor to write to
//     child's stdin, or NULL.
//   - stdoutPipeOut (optional): return location for file descriptor to read
//     child's stdout, or NULL.
//   - stderrPipeOut (optional): return location for file descriptor to read
//     child's stderr, or NULL.
//
func SpawnAsyncWithPipesAndFds(workingDirectory string, argv, envp []string, flags SpawnFlags, childSetup SpawnChildSetupFunc, stdinFd, stdoutFd, stderrFd int, sourceFds, targetFds []int) (childPidOut coreglib.PID, stdinPipeOut, stdoutPipeOut, stderrPipeOut int, goerr error) {
	var _arg1 *C.gchar               // out
	var _arg2 **C.gchar              // out
	var _arg3 **C.gchar              // out
	var _arg4 C.GSpawnFlags          // out
	...
	_arg10 = (*C.gint)(C.calloc(C.size_t(len(sourceFds)), C.size_t(C.sizeof_gint)))
	...
	_arg11 = (*C.gint)(C.calloc(C.size_t(len(targetFds)), C.size_t(C.sizeof_gint)))
...
// SpawnClosePid: on some platforms, notably Windows, the #GPid type
// represents a resource which must be closed to prevent resource leaking.
...
//
func SpawnClosePid(pid coreglib.PID) {
	var _arg1 C.GPid // out
...
//
// #GVariant is heavily optimised for dealing with data in serialised form.
...
// system constraints, such as the number of bits in #gsize. For example,
// it is reasonable to have a 2GB file mapped into memory with File, and call
...
// This section gives a rough idea of how much memory is used by the current
...
// various different possible sources of serialised data that it uses.
// The buffer is responsible for ensuring that the correct call is made when
// the data is no longer in use by #GVariant. This may involve a g_free() or a
...
// # GVariant structure
//
// The size of a #GVariant structure is 6 * (void *). On 32-bit systems,
// that's 24 bytes.
//
// #GVariant structures only exist if they are explicitly created with API
// calls. For example, if a #GVariant is constructed out of serialised data
// for the example given above (with the dictionary) then although there are 9
...
// (ie: until you call g_variant_unref()). The type information is shared.
// The serialised data and the buffer management structure for that serialised
...
// memory for type information, 29 bytes of memory for the serialised data,
// 16 bytes for buffer management and 24 bytes for the #GVariant instance,
// or a total of 160 bytes, plus malloc overhead. If we were to use
...
// As a result, file formats or network messages that incorporate serialised
