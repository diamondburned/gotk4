// Code generated by girgen. DO NOT EDIT.

package gobject

import (
	"runtime/cgo"
	"unsafe"

	externglib "github.com/gotk3/gotk3/glib"
)

// #cgo pkg-config: gobject-2.0 gobject-introspection-1.0
// #cgo CFLAGS: -Wno-deprecated-declarations
// #include <glib-object.h>
// #include <glib-object.h>
import "C"

// BoxedCopy: provide a copy of a boxed structure src_boxed which is of type
// boxed_type.
func BoxedCopy(boxedType externglib.Type, srcBoxed cgo.Handle) cgo.Handle {
	var _arg1 C.GType         // out
	var _arg2 C.gconstpointer // out
	var _cret C.gpointer      // in

	_arg1 = C.GType(boxedType)
	_arg2 = (C.gconstpointer)(unsafe.Pointer(srcBoxed))

	_cret = C.g_boxed_copy(_arg1, _arg2)

	var _gpointer cgo.Handle // out

	_gpointer = (cgo.Handle)(unsafe.Pointer(_cret))

	return _gpointer
}

// BoxedFree: free the boxed structure boxed which is of type boxed_type.
func BoxedFree(boxedType externglib.Type, boxed cgo.Handle) {
	var _arg1 C.GType    // out
	var _arg2 C.gpointer // out

	_arg1 = C.GType(boxedType)
	_arg2 = (C.gpointer)(unsafe.Pointer(boxed))

	C.g_boxed_free(_arg1, _arg2)
}
