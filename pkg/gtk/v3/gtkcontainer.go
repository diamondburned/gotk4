...
// WrapContainer wraps a GObject to a type that implements
// interface Container. It is primarily used internally.
...
func (c container) AsWidget() Widget {
	return WrapWidget(gextras.InternObject(c))
}

func (c container) AsBuildable() Buildable {
	return WrapBuildable(gextras.InternObject(c))
}

func (w container) Activate() bool {
	return WrapWidget(gextras.InternObject(w)).Activate()
}

func (w container) AddAccelerator(accelSignal string, accelGroup AccelGroup, accelKey uint, accelMods gdk.ModifierType, accelFlags AccelFlags) {
	WrapWidget(gextras.InternObject(w)).AddAccelerator(accelSignal, accelGroup, accelKey, accelMods, accelFlags)
}

func (w container) AddDeviceEvents(device gdk.Device, events gdk.EventMask) {
	WrapWidget(gextras.InternObject(w)).AddDeviceEvents(device, events)
}

func (w container) AddEvents(events int) {
	WrapWidget(gextras.InternObject(w)).AddEvents(events)
}

func (w container) AddMnemonicLabel(label Widget) {
	WrapWidget(gextras.InternObject(w)).AddMnemonicLabel(label)
}

func (w container) CanActivateAccel(signalId uint) bool {
	return WrapWidget(gextras.InternObject(w)).CanActivateAccel(signalId)
}

func (w container) ChildFocus(direction DirectionType) bool {
	return WrapWidget(gextras.InternObject(w)).ChildFocus(direction)
}

func (w container) ChildNotify(childProperty string) {
	WrapWidget(gextras.InternObject(w)).ChildNotify(childProperty)
}

func (w container) ClassPath() (pathLength uint, path string, pathReversed string) {
	return WrapWidget(gextras.InternObject(w)).ClassPath()
}

func (w container) ComputeExpand(orientation Orientation) bool {
	return WrapWidget(gextras.InternObject(w)).ComputeExpand(orientation)
}

func (w container) CreatePangoContext() pango.Context {
	return WrapWidget(gextras.InternObject(w)).CreatePangoContext()
}

func (w container) CreatePangoLayout(text string) pango.Layout {
...
func (b container) GetInternalChild(builder Builder, childname string) gextras.Objector {
	return WrapBuildable(gextras.InternObject(b)).GetInternalChild(builder, childname)
}

func (b container) GetName() string {
