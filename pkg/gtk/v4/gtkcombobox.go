...
func classInitComboBoxer(gclassPtr, data C.gpointer) {
	...
	*(*C.gpointer)(unsafe.Add(unsafe.Pointer(gclassPtr), goffset)) = data
	...
	if _, ok := goval.(interface{ Changed() }); ok {
		...
		*(*unsafe.Pointer)(unsafe.Add(unsafe.Pointer(gclassPtr), o)) = unsafe.Pointer(C._gotk4_gtk4_ComboBoxClass_changed)
	}

	if _, ok := goval.(interface{ FormatEntryText(path string) string }); ok {
		o := pclass.StructFieldOffset("format_entry_text")
		*(*unsafe.Pointer)(unsafe.Add(unsafe.Pointer(gclassPtr), o)) = unsafe.Pointer(C._gotk4_gtk4_ComboBoxClass_format_entry_text)
	}
}
