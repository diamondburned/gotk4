...
func (context *PrintContext) HardMargins() (top, bottom, left, right float64, ok bool) {
	...
	*(**C.void)(unsafe.Pointer(&_args[0])) = (*C.void)(unsafe.Pointer(coreglib.InternObject(context).Native()))
	...
	_top = float64(*(*C.double)(unsafe.Pointer(&_outs[0])))
	_bottom = float64(*(*C.double)(unsafe.Pointer(&_outs[1])))
	_left = float64(*(*C.double)(unsafe.Pointer(&_outs[2])))
	_right = float64(*(*C.double)(unsafe.Pointer(&_outs[3])))
