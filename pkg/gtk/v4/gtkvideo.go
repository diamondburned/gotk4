...
// WrapVideo wraps a GObject to a type that implements
// interface Video. It is primarily used internally.
...
func (w video) SnapshotChild(child Widget, snapshot Snapshot) {
	WrapWidget(gextras.InternObject(w)).SnapshotChild(child, snapshot)
}

func (s video) TranslateCoordinates(destWidget Widget, srcX float64, srcY float64) (destX float64, destY float64, ok bool) {
	return WrapWidget(gextras.InternObject(s)).TranslateCoordinates(destWidget, srcX, srcY)
}

func (w video) TriggerTooltipQuery() {
...
func (b video) GetBuildableID() string {
